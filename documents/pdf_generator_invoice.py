# documents/pdf_generator_invoice.py — PLAGENOR 4.0 Invoice PDF Generator
# Updated with QR code support
...
def generate_invoice_pdf(invoice):
    ...
    try:
        ...
        qr_data = generate_qr_data_url(f"INV-{invoice.invoice_number}")
        if qr_data:
            import base64
            from reportlab.lib.utils import ImageReader
            qr_parts = qr_data.split(',')
            if len(qr_parts) >= 2:
                qr_data_clean = qr_parts[1]
                qr_bytes = base64.b64decode(qr_data_clean)
                qr_img = ImageReader(BytesIO(qr_bytes))
                # Create header row with title and QR code
                title_col = Paragraph("FACTURE / INVOICE", styles['Title2'])
                qr_col = Image(qr_img, width=2*cm, height=2*cm)
                qr_table = Table([[title_col, qr_col]], colWidths=[14*cm, 3*cm])
        ...
        # Fallback without QR
        title_col = Paragraph("FACTURE / INVOICE", styles['Title2'])
        qr_table = Table([[title_col, '']], colWidths=[14*cm, 3*cm])
        qr_table.setStyle(TableStyle([
            ('ALIGN', (0, 0), (0, 0), 'LEFT'),
            ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
        ]))
